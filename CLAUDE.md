...
# OpenCAT Studio — Project Notes

## Tool paths (Laravel Herd)

This project requires **PHP 8.4** (starter kit was compiled against it).
...
PostgreSQL 16 required (`pg_trgm` extension). Connection config: `config/database.php` → `pgsql`.
Create the database before running `artisan migrate`: `createdb opencat_studio`

## Queue

Use `QUEUE_CONNECTION=sync` for local development (no worker needed).
Production: `redis` with `php artisan queue:work redis --queue=critical,default,exports`
